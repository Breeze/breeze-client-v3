...
import { DataService, JsonResultsAdapter, NodeContext, NodeMeta } from '../metadata/data-service.js';
import { EntityState  } from '../entity/entity-state.js';
import { EntityAction } from '../entity/entity-action.js';
import { MetadataStore, EntityType, StructuralType, DataProperty, NavigationProperty } from '../metadata/entity-metadata.js';
...
function updateEntityRef(mc: MappingContext, targetEntity: any, node: any) {
  ...
  if (!nodeId && node._$meta.extraMetadata) {
    // refMap is not really used, but is returned as data.retrievedEntities, so we populate it anyway.
...
function mergeRelatedEntitiesCore(mc: MappingContext, rawEntity: any, navigationProperty: NavigationProperty) {
  ...
  // a navigation property is expected to yield an array
