...
import { EntityAspect, ComplexAspect, Entity, ComplexObject, StructuralObject } from '../entity/entity-aspect.js';
...
export class MetadataStore {
 ...
 getAsComplexType(typeName: string, okIfNotFound: boolean = false) {
  ...
  @deprecated Misleading name: it returns an EntityType *or* a ComplexType. Use
  {@link MetadataStore.getAsEntityType} or {@link MetadataStore.getAsComplexType} when you know
  which kind you expect, and {@link MetadataStore.getStructuralType} when either will do.
  @param typeName - Either the fully qualified name or a short name may be used. If a short name is specified and multiple types share
  that same short name an exception will be thrown.
  @param okIfNotFound - (default=false) Whether to throw an error if the specified EntityType is not found.
  @returns The EntityType. ComplexType or 'null' if not not found.
...
// The model library Breeze falls back to when none is registered (see default-adapters.ts).
// Installed here because a MetadataStore needs one even in a bundle with no EntityManager, and
// this module is in every bundle that has entity types. A getter, because the adapter's module
// imports this one: while this module loads, the adapter class may not exist yet.
setDefaultAdapters({ modelLibrary: { get ctor() { return ModelLibraryBackingStoreAdapter; } } });
  ...
  */
  ...
  resolveProperty(propName: string) {
    ...
    while (result == undefined && baseProp != null) {
      result = (baseProp as Record<string, any>)[propName];
...
(Error as any)['x'] = AutoGeneratedKeyType.resolveSymbols();
