...
The generated classes use what lives in `breeze-client` — `RelationArray` and `ComplexArray`
for collection properties, and the `EntityBase` / `ComplexObjectBase` they extend — and they
import it from `breeze-client`, which needs no saying: it is right wherever the package is installed, this repo included. `--breeze`
...
`instanceof` checks that fail for no visible reason. Every class extends `EntityBase` or
`ComplexObjectBase`, a real import, so the model does load Breeze through that name.
`test/unit/model-classes.spec.ts` asserts the two spellings give the same module.
...
scaffolds `app-entity-base.ts` once — extending Breeze's `EntityBase`, so the
...
`index.ts` carries a header saying it is generated whole. Anything you want to keep belongs in
another module.
...
That is the hook for a version that has to migrate what an older one emitted. v1.1.0 is the first
to use it: v1.0.0 wrote `EntityBase` and `ComplexObjectBase` into a generated `entity-base.ts`,
and they now ship in Breeze. A marked import of them from `./entity-base` is re-pointed to
`breeze-client`, and an `entity-base.ts` carrying the generator's header becomes a re-export,
because a scaffolded `--base` class imports from it and is never rewritten. A fresh model gets
no `entity-base.ts`, and neither does this one.
...
- **Breeze-supplied members live on `EntityBase` / `ComplexObjectBase`**, exported by
  `breeze-client`: `entityAspect`, `entityType`, `getProperty`, `setProperty`, and the
  complex-type equivalents.
