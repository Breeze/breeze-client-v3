...
describe("Entity Creation - no server", () => {
  ...
  test("createEntity - 2", () => {
    ...
    expect(emp3.lastName).toBe("Smith");
  });
  ...
  test("createEntity merge new into deleted entity", function () {
    ...
    const sameCust = em.createEntity("Customer", { customerID: id }, null, MergeStrategy.OverwriteChanges) as Customer;
    expect(sameCust.entityAspect.entityState).toBe(EntityState.Added);
    expect(sameCust.customerID).toBe(id);
  ...
  test("create entity with non-null dates", function () {
    ...
    const user = userType.createEntity() as User;

    const crtnDate = user.createdDate;
    const modDate = user.modifiedDate;
    ...
    const user2 = em2.getEntityByKey("User", id) as User;
    const crtnDate2 = user2.createdDate;
    const modDate2 = user2.modifiedDate;
  ...
  test("create entity with initial properties", function () {
    ...
    const employee = empType.createEntity(cfg) as Employee;
    ...
    cfg[employeeKeyName] = TestFns.wellKnownData.dummyEmployeeID;
    const partialEmp = empType.createEntity(cfg) as Employee;
  ...
  test("post create init 1", () => {
    ...
    expect(product.isObsolete).toBe(true);

    product.isObsolete = false;
    expect(product.isObsolete).toBe(false);
  });
  ...
  test("attaching entities in ctor makes fk values update", function () {
    ...
    expect(sup.supplierID).toBeLessThan(0);
    const prods = sup.products;
    expect(prods[0].productID).toBeLessThan(0);
    expect(prods[0].supplierID).toBeLessThan(0);
    expect(prods[1].productID).toBeLessThan(0);
    expect(prods[1].supplierID).toBeLessThan(0);
  });
  ...
  test("attachEntity - store-managed int ID remains '0'", () => {
    ...
    const emp = employeeType.createEntity() as Employee;
  ...
  test("addEntity - no key", function () {
    ...
    const od = odType.createEntity() as OrderDetail;
  ...
  test("addEntity - child", () => {
    ...
    const orders = cust1.orders as RelationArray;
  ...
  test("addEntity - detach child", () => {
    ...
    const sameCust = order2.customer;
