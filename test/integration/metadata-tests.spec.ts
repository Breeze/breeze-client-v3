import { Entity, EntityQuery, EntityType, MetadataStore, EntityChangedEventArgs, EntityAction, MergeStrategy, QueryOptions, FetchStrategy, EntityManager, breeze, DataProperty, AutoGeneratedKeyType, NamingConvention, core, config, DataServiceAdapter, DataService, NavigationProperty } from '../../src/breeze';
...
describe("Metadata", () => {
  ...
  test("export/import custom metadata", function () {
    ...
    const custType = store.getAsEntityType("Customer");
    const namespace = custType.namespace;
    expect(store.hasMetadataFor(TestFns.defaultServiceName)).toBeTruthy();
    const customMetadata = makeCustomMetadata(namespace);
    store.importMetadata(customMetadata, true);
  ...
  function createEmWithTimeGroupMetadata(addFooBar?: any) {
    ...
    const nc = NamingConvention.camelCase;
